...
bool Clk_Wifi::begin(Config *configuration) {
  ...
  if (wifiConfigLoaded) {
    ...
    DBG_VERBOSE("%s begin ------^^------ WIFI: Adding APs ------^^------\n", FILE_NAME_WIFI);
    for (NetworkInfo netinfo : config->networks) {
      ...
      if (ssid[0] != '\0') {
        ...
        DBG_VERBOSE("%s begin ------^^------ WIFI: Added AP %s ------^^------\n", FILE_NAME_WIFI, ssid);
      }
    }
  }
  ...
  if (apAddedCount == 0) {
    ...
    DBG_VERBOSE("%s begin ------^^------ WIFI: Starting AP Mode ------^^------\n", FILE_NAME_WIFI);
    ...
    DBG_VERBOSE("%s begin - Connecting WiFi...\n", FILE_NAME_WIFI);
    ...
    while (attempt < MAX_ATTEMPTS) {
      if (wifiMulti.run() == WL_CONNECTED) {
        connected = true;
        DBG_VERBOSE("%s begin - WiFi connected, SSID: %s\n", FILE_NAME_WIFI, WiFi.SSID().c_str());
        // Optimize: Avoid temporary String object by formatting IP directly
        IPAddress ip = WiFi.localIP();
        DBG_VERBOSE("%s begin - IP address: %d.%d.%d.%d\n", FILE_NAME_WIFI, ip[0], ip[1], ip[2], ip[3]);
        DBG_VERBOSE("%s begin - Connected in %d ms\n", FILE_NAME_WIFI, millis() - startConnect);
        break;
      }
      
      attempt++;
      if (attempt < MAX_ATTEMPTS) {
        DBG_VERBOSE("%s begin - Attempt %d/%d failed. Retrying...\n", FILE_NAME_WIFI, attempt, MAX_ATTEMPTS);
        delay(1000);
      }
    }
    
    if (!connected) {
      DBG_VERBOSE("%s begin - Failed to connect after %d attempts\n", FILE_NAME_WIFI, MAX_ATTEMPTS);
    }

    DBG_VERBOSE("%s begin ------^^------ WIFI: Starting Station Mode ------^^------\n", FILE_NAME_WIFI);
...
void Clk_Wifi::checkConnectionAccessPointMode(bool fromBegin) {
  ...
  if (WiFi.getMode() == WIFI_AP || WiFi.getMode() == WIFI_AP_STA) {
    DBG_VERBOSE("%s checkConnectionAccessPointMode - AP Mode active\n", FILE_NAME_WIFI);
  } else {
    DBG_VERBOSE("%s checkConnectionAccessPointMode - WARNING: AP Mode not active\n", FILE_NAME_WIFI);
  }
}
...
void Clk_Wifi::checkConnectionStationMode(bool fromBegin) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    DBG_VERBOSE("%s checkConnectionStationMode - Already connected to %s\n", 
           FILE_NAME_WIFI, WiFi.SSID().c_str());
    return;
  }
  
  // Attempt to reconnect
  DBG_VERBOSE("%s checkConnectionStationMode - Connection lost, attempting to reconnect...\n", FILE_NAME_WIFI);
  
  if (wifiMulti.run() == WL_CONNECTED) {
    DBG_VERBOSE("%s checkConnectionStationMode - Reconnected to %s\n", 
           FILE_NAME_WIFI, WiFi.SSID().c_str());
    // Optimize: Avoid temporary String object by formatting IP directly
    IPAddress ip = WiFi.localIP();
    DBG_VERBOSE("%s checkConnectionStationMode - IP address: %d.%d.%d.%d\n", 
           FILE_NAME_WIFI, ip[0], ip[1], ip[2], ip[3]);
  } else {
    DBG_VERBOSE("%s checkConnectionStationMode - Failed to reconnect\n", FILE_NAME_WIFI);
  }
}
